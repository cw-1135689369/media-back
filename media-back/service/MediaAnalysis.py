...
def getDataByDate(date):
    # 获取数据
    df = pd.read_csv("./static/datas/media_analysis.csv")
    # 新增时间类型的列
...
def getHostilityRatio(date, media):
    ...
    # 对字符串进行处理,得到列表中所有字符串
    for negative_conn in negative_counnt_data:
        str = str + negative_conn
    # 去除字符串的开头的空白
    str = str[1:len(str)]
    # 将字符串切割插入列表
    data = str.split(" ")
...
def getDataForMediaAnalysis(media, date):
    ...
    if average_count >= 1:
        if dispersion_coefficient <= 0.8:
            stability_evaluation = "非常稳定"
        if 0.8 < dispersion_coefficient <= 1.2:
            stability_evaluation = "稳定"
        if dispersion_coefficient > 1.2:
            stability_evaluation = "不稳定"
    else:
        if count_data_new.max() - count_data_new.min() <= 3:
            stability_evaluation = "非常稳定"
        if 3 < count_data_new.max() - count_data_new.min() <= 6:
            stability_evaluation = "非常稳定"
        if count_data_new.max() - count_data_new.min() > 3:
            stability_evaluation = "非常稳定"
    # 获得工作日发文量 非工作日发文量
