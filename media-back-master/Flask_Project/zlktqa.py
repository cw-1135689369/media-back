...
def getMediaNegativeNewsListByreporter():
    reporter= request.args.get("reporter")
    date=request.args.get("date", type=int)
    u = ReporterAnalysis.getMediaNegativeNewsListByreporter(reporter, date)
    return jsonify(u)
# 获取记者正面关注新闻
...
def get_data_job():
    ...
    # 将数据转化成dataframe
    df_media_analysis = pd.DataFrame(s, columns=['post_id','media', 'reporter','str_date','title','link','scores','grp_copy','key1','category'])
